...
当前版本：**1.3.5**（Factory）/ **1.3.5l**（M5Launcher）
...
| Unicode GATT 注入 | 仅 macOS | Windows 1.3.5 暂不提供 |

## Cardputer 固件刷入

需要 Cardputer 和可传输数据的 USB-C 线。请先选择一个安装通道：

- **Factory 1.3.5（推荐）**：从 `0x0` 写入官方产品分区表。会移除
  M5Launcher，并重置 Wi-Fi、PIN、键盘 Profile、宠物、初始化状态和 BLE 配对。
- **Launcher 1.3.5l**：保留 M5Launcher 及其 Flash 布局。先把
  M5Launcher 更新到 **2.8.0 或更高版本**，再通过 M5Launcher 安装
  `Cardputer-Codex-Companion-1.3.5l-launcher.bin`。后续更新仍通过
  M5Launcher 完成。
...
通过 USB 连接 Cardputer，然后选择 **Install Factory Firmware 1.3.5**。
...
`Cardputer-Codex-Companion-1.3.5l-launcher.bin`，再从 M5Launcher
...
镜像，或改用 Factory 1.3.5。Web 会把分区不兼容明确显示为存储错误，不会把
有效 PIN 错报为“PIN 错误”。
...
Windows ARM64 解压 `CardputerCompanion-1.3.5-windows-arm64.zip` 后运行：
...
PIN 由当前 Windows 用户的 DPAPI 保护。可从“已安装的应用”或开始菜单卸载。Windows 1.3.5 不包含 Unicode GATT 注入和蓝牙麦克风。
...
公共制品包括 Factory 1.3.5 固件、应用升级固件、Launcher 1.3.5l 固件、
