...
Current release: **1.3.3** (Factory) / **1.3.3l** (M5Launcher)
...
| Unicode GATT injection | macOS only | Windows support is not included in 1.3.3 |
...
- **Factory 1.3.3 (recommended):** installs the official product partition
  layout at offset `0x0`. It removes M5Launcher and resets Wi-Fi, PIN, keyboard
  profiles, pets, onboarding state, and BLE pairing.
- **Launcher 1.3.3l:** preserves M5Launcher and its flash layout. Update
  M5Launcher to **2.8.0 or later**, then install
  `Cardputer-Codex-Companion-1.3.3l-launcher.bin` through M5Launcher. Use
...
**Install Factory Firmware 1.3.3**.
...
`Cardputer-Codex-Companion-1.3.3l-launcher.bin`, and install it from
...
and reinstall the Launcher artifact, or switch to Factory 1.3.3. A valid PIN
...
For Windows ARM64, extract `CardputerCompanion-1.3.3-windows-arm64.zip` and run:
...
Windows 1.3.3 does not include Unicode GATT injection or the Bluetooth microphone.
...
- `dist/Cardputer-Codex-Companion-1.3.3-app.bin`;
- `dist/Cardputer-Codex-Companion-1.3.3l-launcher.bin`;
...
- `dist/CardputerCompanion-1.3.3-web-installer.zip`;
- `dist/1.3.3-SHA256SUMS`.
