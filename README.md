...
Current release: **1.3.5** (Factory) / **1.3.5l** (M5Launcher)
...
| Unicode GATT injection | macOS only | Windows support is not included in 1.3.5 |
...
- **Factory 1.3.5 (recommended):** installs the official product partition
  layout at offset `0x0`. It removes M5Launcher and resets Wi-Fi, PIN, keyboard
  profiles, pets, onboarding state, and BLE pairing.
- **Launcher 1.3.5l:** preserves M5Launcher and its flash layout. Update
  M5Launcher to **2.8.0 or later**, then install
  `Cardputer-Codex-Companion-1.3.5l-launcher.bin` through M5Launcher. Use
  M5Launcher for later updates.

Verify the downloaded files before flashing:

```bash
shasum -a 256 -c 1.3.5-SHA256SUMS
```

### Factory 1.3.5 (recommended)
...
**Install Factory Firmware 1.3.5**.
...
python3 -m esptool --chip esp32s3 \
  ...
  write_flash 0x0 Cardputer-Codex-Companion-1.3.5-factory.bin
```
...
`Cardputer-Codex-Companion-1.3.5l-launcher.bin`, and install it from
...
and reinstall the Launcher artifact, or switch to Factory 1.3.5. A valid PIN
...
python3 -m esptool --chip esp32s3 \
  --port /dev/cu.usbmodemXXXX -b 460800 \
  --before default_reset --after hard_reset \
  write_flash 0x20000 Cardputer-Codex-Companion-1.3.5-app.bin
```
...
```text
CardputerCompanion-1.3.5-windows-x64-setup.exe
```
...
For Windows ARM64, extract `CardputerCompanion-1.3.5-windows-arm64.zip` and run:
...
Windows 1.3.5 does not include Unicode GATT injection or the Bluetooth microphone.
...
- `dist/Cardputer-Codex-Companion-1.3.5-factory.bin`;
- `dist/Cardputer-Codex-Companion-1.3.5-app.bin`;
- `dist/Cardputer-Codex-Companion-1.3.5l-launcher.bin`;
...
- `dist/CardputerCompanion-1.3.5-web-installer.zip`;
- `dist/1.3.5-SHA256SUMS`.
