...
Current release: **1.3.4** (Factory) / **1.3.4l** (M5Launcher)
...
| Unicode GATT injection | macOS only | Windows support is not included in 1.3.4 |
...
- **Factory 1.3.4 (recommended):** installs the official product partition
  layout at offset `0x0`. It removes M5Launcher and resets Wi-Fi, PIN, keyboard
  profiles, pets, onboarding state, and BLE pairing.
- **Launcher 1.3.4l:** preserves M5Launcher and its flash layout. Update
  M5Launcher to **2.8.0 or later**, then install
  `Cardputer-Codex-Companion-1.3.4l-launcher.bin` through M5Launcher. Use
...
**Install Factory Firmware 1.3.4**.
...
`Cardputer-Codex-Companion-1.3.4l-launcher.bin`, and install it from
...
and reinstall the Launcher artifact, or switch to Factory 1.3.4. A valid PIN
...
For Windows ARM64, extract `CardputerCompanion-1.3.4-windows-arm64.zip` and run:
...
Windows 1.3.4 does not include Unicode GATT injection or the Bluetooth microphone.
