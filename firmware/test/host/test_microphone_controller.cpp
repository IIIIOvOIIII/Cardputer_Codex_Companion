...
int main() {
  ...
  MicrophoneController controller(capture, transport, 0xFFFF);
  make_live(controller, capture);

  assert(controller.run_once());
  assert(capture.last_read_size == 240);
  ...
  assert(controller.run_once());
  assert(capture.last_read_size == 160);
  ...
  MicrophoneController disconnected(disconnected_capture,
  ...
  MicrophoneController not_ready(not_ready_capture,
                                 not_ready_transport);
